...
"""
    ...
    private func fileContentDescription(for ref: FileRef) -> String {
        ...
        if let string = String(data: data, encoding: .utf8) ??
            ...
            if trimmed.count > 4000 {
                let prefix = trimmed.prefix(4000)
                capped = "\(prefix)\n...(truncated)"
            } else {
                capped = trimmed
            }
    ...
    private func openAIMessages(from history: [ChatMsg], systemPrompts: [String]) -> [AIService.Message] {
        ...
        let previousImages = history.dropLast().last?.images ?? []
        ...
        for msg in history {
            ...
            if trimmed.isEmpty && !hasImages { continue }
            let role = msg.role == .user ? "user" : "assistant"
            if msg.role == .user {
                var parts: [AIService.Message.ContentPart] = []
    ...
    private func imageDataURL(for ref: ImageRef) -> String? {
        guard let url = imageURL(for: ref) else {
            return nil
        }
        let ext = url.pathExtension.lowercased()
        ...
        case "webp":
            ...
            data = try? Data(contentsOf: url)
        default:
            // For unsupported types like HEIC, TIFF, etc., convert to JPEG.
            if let image = NSImage(contentsOf: url),
               let jpegData = jpegData(from: image, quality: 0.9) {
                data = jpegData
                mimeType = "image/jpeg"
            } else {
