...
struct ReasoningItem: Codable, Identifiable {
    var id: UUID
    var ts: Double
    var messageId: UUID
    var reasoningTokens: Int
    var reasoningContent: String  // The actual reasoning text
    
    var tokenSummary: String {
        if reasoningTokens == 0 {
            return "No reasoning tokens"
        } else if reasoningTokens < 100 {
            return "\(reasoningTokens) tokens (brief)"
        } else if reasoningTokens < 500 {
            return "\(reasoningTokens) tokens (moderate)"
        } else if reasoningTokens < 1000 {
            return "\(reasoningTokens) tokens (extended)"
        } else {
            return "\(reasoningTokens) tokens (deep)"
        }
    }
}
