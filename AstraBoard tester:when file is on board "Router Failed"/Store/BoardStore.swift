...
- [text] id: 123 text: ...
- [image] id: 456
- [file] id: 789 name: notes.md
Board images may be attached after the payload with labels like "Board image id: <id>".
Board file text may be attached after the payload with labels like "Board file id: <id>" followed by extracted text (possibly truncated).
...
"""
    ...
    - Do NOT infer. If the user didn't explicitly say it, don't store it.
    ...
    - Use DELETE only when the user explicitly says a memory is no longer true AND provides no replacement.
    ...
    - If the user restates a memory with the same meaning, do nothing.
    ...
    - Each memory string is a single sentence in plain language.
    ...
    private func runOrchestrator(replyId: UUID,
        ...
        var boardInjection = decision.boardSelection.boardInjection.trimmingCharacters(in: .whitespacesAndNewlines)
        if boardInjection.isEmpty {
            boardInjection = defaultBoardInjection(from: selectedEntries)
        }
        if !boardInjection.isEmpty {
        ...
        let selectedBoardFiles = boardFileAttachments(from: selectedEntries)
        if !selectedBoardFiles.isEmpty {
            var parts: [AIService.Message.ContentPart] = [
                .text("Selected board files (context only; not user message):")
            ]
            for attachment in selectedBoardFiles {
                parts.append(.text("Board file id: \(attachment.id.uuidString)"))
                parts.append(.text(attachment.content))
            }
            messagesForAPI.append(AIService.Message(role: "user", content: .parts(parts)))
        }
    ...
    private func routerMessages(for userText: String,
        ...
        let routerFileAttachments = boardFileAttachmentsForRouting(entries: boardEntries,
                                                                   order: boardOrder,
                                                                   selection: selection)

        let userContent: AIService.Message.Content
        if routerImageAttachments.isEmpty && routerFileAttachments.isEmpty {
            ...
            if !routerFileAttachments.isEmpty {
                parts.append(.text("Board files (context only; not user message):"))
                for attachment in routerFileAttachments {
                    parts.append(.text("Board file id: \(attachment.id.uuidString)"))
                    parts.append(.text(attachment.content))
                }
            }
    ...
    private func boardEntryContextLine(for entry: BoardEntry,
        ...
        case .file(let ref):
            ...
            if selected {
                let preview = fileContentDescription(for: ref, maxChars: 1200)
    ...
    private func selectedBoardEntries(from selectedIds: [String],
        ...
        for rawId in selectedIds {
            let trimmed = rawId.trimmingCharacters(in: .whitespacesAndNewlines)
            guard let id = UUID(uuidString: trimmed),
                  let entry = entries[id] else { continue }
    ...
    private func boardEntryInjectionLine(for entry: BoardEntry) -> String {
        ...
        case .file(let ref):
            let name = ref.originalName.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = name.isEmpty ? ref.filename : name
            return "- [file] id: \(entry.id.uuidString) name: \(label)"
        default:
            return ""
        }
    }
    ...
    private func boardImageAttachments(from entries: [BoardEntry],
                                      maxPixelSize: CGFloat = 2048,
                                      quality: CGFloat = 0.85) -> [BoardImageAttachment] {
        guard !entries.isEmpty else { return [] }
        var out: [BoardImageAttachment] = []

        for entry in entries {
            guard case .image(let ref) = entry.data else { continue }

            if let dataURL = routerImageDataURL(for: ref, maxPixelSize: maxPixelSize, quality: quality)
                ?? imageDataURL(for: ref) {
                out.append(BoardImageAttachment(id: entry.id, dataURL: dataURL))
            }
        }
    ...
    /// Router gets a small set of board images (within caps) so it can decide relevance by ID.
    private func boardImageAttachmentsForRouting(entries: [UUID: BoardEntry],
                                                order: [UUID],
                                                selection: Set<UUID>,
                                                maxPixelSize: CGFloat = 512,
                                                quality: CGFloat = 0.6,
                                                maxImages: Int = 6) -> [BoardImageAttachment] {
        guard !entries.isEmpty else { return [] }
        ...
        var out: [BoardImageAttachment] = []
        out.reserveCapacity(min(maxImages, allIds.count))

        for id in allIds {
            guard let entry = entries[id],
                  case .image(let ref) = entry.data else { continue }

            guard let dataURL = routerImageDataURL(for: ref, maxPixelSize: maxPixelSize, quality: quality)
                    ?? imageDataURL(for: ref) else { continue }

            out.append(BoardImageAttachment(id: id, dataURL: dataURL))
            if out.count >= maxImages { break }
        }

        return out
    }
    
    private struct BoardFileAttachment {
        let id: UUID
        let content: String
    }

    private func boardFileAttachments(from entries: [BoardEntry],
                                      maxCharsPerFile: Int = 23000) -> [BoardFileAttachment] {
        guard !entries.isEmpty else { return [] }
        var out: [BoardFileAttachment] = []
        for entry in entries {
            guard case .file(let ref) = entry.data else { continue }
            let content = fileContentDescription(for: ref, maxChars: maxCharsPerFile)
            out.append(BoardFileAttachment(id: entry.id, content: content))
        }
        return out
    }

    /// Router gets a preview of EVERY board file (within caps) so it can decide relevance by ID.
    private func boardFileAttachmentsForRouting(entries: [UUID: BoardEntry],
                                                order: [UUID],
                                                selection: Set<UUID>,
                                                maxCharsPerFile: Int = 1200,
                                                maxTotalChars: Int = 12000) -> [BoardFileAttachment] {
        guard !entries.isEmpty else { return [] }
        ...
        for id in allIds {
            guard let entry = entries[id],
                  case .file(let ref) = entry.data else { continue }

            let content = fileContentDescription(for: ref, maxChars: maxCharsPerFile)
            let cost = content.count + 80 // small overhead for labels
            if total + cost > maxTotalChars {
                break
            }

            out.append(BoardFileAttachment(id: id, content: content))
            total += cost
        }
    ...
    private func fileContentDescription(for ref: FileRef, maxChars: Int = 4000) -> String {
        let name = ref.displayName
        guard let url = fileURL(for: ref),
        ...
        if let typeId = (try? url.resourceValues(forKeys: [.typeIdentifierKey]).typeIdentifier),
           ...
           type.conforms(to: .pdf) {
            if let pdf = PDFDocument(url: url) {
                ...
                if !pdfText.isEmpty {
                    let capped: String
                    if pdfText.count > maxChars {
                        let prefix = pdfText.prefix(maxChars)
                        capped = "\(prefix)\n...(truncated)"
                    } else {
                        capped = pdfText
                    }
        ...
        if let string = String(data: data, encoding: .utf8) ??
            ...
            if trimmed.count > maxChars {
                let prefix = trimmed.prefix(maxChars)
                capped = "\(prefix)\n...(truncated)"
            } else {
                capped = trimmed
            }
