...
extension BoardStore {
    ...
    private func restore(snapshot: BoardSnapshot) {
        ...
        chatDraftImage = snapshot.chatDraftImage
...
extension BoardStore {
    ...
    func clearChatDraftImage() {
        guard chatDraftImage != nil else { return }
        recordUndoSnapshot()
        chatDraftImage = nil
    }
...
extension BoardStore {
    ...
    func sendChat(text: String, image: ImageRef? = nil) -> Bool {
        ...
        let routedText = clarificationMergedText(original: pendingClarification?.originalText,
                                                 ...
                                                 clarificationHasImage: image != nil)
            ?? messageText
        let routedImage = image ?? pendingClarification?.originalImage
    ...
    func editChatMessageAndResend(messageId: UUID, text: String) {
        ...
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        let messageImage = doc.chat.messages[index].image
        guard !trimmed.isEmpty || messageImage != nil else { return }
        let messageText = trimmed.isEmpty ? "" : text
        recordUndoSnapshot()
        chatWarning = nil
    ...
    func retryChatReply(messageId: UUID) {
        ...
        doc.chat.messages[index].image = nil
        doc.chat.messages[index].ts = Date().timeIntervalSince1970
        pendingChatReplies += 1
...
Board:
You will be given board entries (text and image elements) with IDs. Select only relevant entry IDs into selected_entry_ids.
...
"""
    ...
    private func runOrchestrator(replyId: UUID,
        ...
        if intents.contains("image_edit") {
            let editImage = image ?? selectedImageRef
        ...
        if let imageRef = generatedImageRef {
            if await handleChatCancellation(replyId: replyId) { return }
            await MainActor.run {
                setChatReplyImage(replyId: replyId, imageRef: imageRef)
            }
        }
        if await handleChatCancellation(replyId: replyId) { return }
        await MainActor.run {
    ...
    private func routerMessages(for userText: String,
        ...
        lines.append("USER_MESSAGE>>>")
        lines.append("Has image attachment: \(imageAttached ? "true" : "false")")
    ...
    private func userMessage(text: String, image: ImageRef?) -> AIService.Message {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        var parts: [AIService.Message.ContentPart] = []
    ...
    private func clarificationMergedText(original: String?,
                                         ...
                                         clarificationHasImage: Bool) -> String? {
        ...
        } else if clarificationHasImage {
            lines.append("User clarification: provided an image.")
        }
        return lines.joined(separator: "\n")
    }
    ...
    @MainActor
    private func setChatReplyImage(replyId: UUID, imageRef: ImageRef?) {
        guard let index = doc.chat.messages.firstIndex(where: { $0.id == replyId }) else { return }
