...
struct PanelsState: Codable {
    ...
    init(chat: PanelBox, chatArchive: PanelBox, log: PanelBox, thoughts: PanelBox, memories: PanelBox, shapeStyle: PanelBox, settings: PanelBox, personality: PanelBox) {
        self.chat = chat
    ...
    init(from decoder: Decoder) throws {
        ...
        memories = try container.decodeIfPresent(PanelBox.self, forKey: .memories)
            ?? PanelBox(isOpen: false, x: 380, y: 220, w: 320, h: 300)
...
extension BoardDoc {
    static func defaultDoc() -> BoardDoc {
        ...
        let panels = PanelsState(chat: PanelBox(isOpen: false, x: 260, y: 120, w: 320, h: 400),
                                 ...
                                 memories: PanelBox(isOpen: false, x: 380, y: 220, w: 320, h: 300),
...
struct RouterDecision: Decodable {
    struct MemorySelection: Decodable {
        ...
        init(from decoder: Decoder) throws {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            selectedMemories = (try? container.decode([String].self, forKey: .selectedMemories)) ?? []
            memoryInjection = (try? container.decode(String.self, forKey: .memoryInjection)) ?? ""
        }
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        memorySelection = (try? container.decode(MemorySelection.self, forKey: .memorySelection)) ?? MemorySelection(selectedMemories: [], memoryInjection: "")
