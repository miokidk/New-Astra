...
You are a routing model. Your task is to create a plan to fulfill the user's request.

You must return a JSON object with a single key, "steps".
"steps" is an array of objects, where each object represents a step in the plan.
Each step object must have the following keys:
- "tool": The tool to use for this step. Can be "text", "image", or "web_search".
- "prompt": The prompt to use for this step.
- "save_as": (Optional) A variable name to save the output of this step to.

The output of a step can be used in a subsequent step by using the `{{variable_name}}` syntax in the prompt.

For example, if the user asks for "a story about a cat and an image of the cat", the plan could be:
{
  "steps": [
    {
      "tool": "text",
    ...
    {
      "tool": "image",
      "prompt": "An image of a cat from the following story: {{cat_story}}"
    }
  ]
}

Now, create a plan for the user's request. Return only the JSON object.
"""
    ...
    private func runOrchestrator(replyId: UUID,
        ...
        // 1. Get Plan from Router
        let plan: Plan
        do {
            plan = try await getPlan(userText: userText,
                                         images: images,
                                         files: files,
                                         apiKey: apiKey,
                                         history: history,
        ...
        var finalImages: [ImageRef] = []

        for step in plan.steps {
            if await handleChatCancellation(replyId: replyId) { return }

            do {
                let prompt = substituteVariables(in: step.prompt, with: stepOutputs)
                
                switch step.tool {
                case "text":
                    let textModel = complexTextModelName
                    ...
                    messagesForAPI.append(AIService.Message(role: "user", content: .text(prompt)))

                    let result = try await aiService.completeChat(model: textModel,
                                                                  apiKey: apiKey,
                                                                  messages: messagesForAPI)
                    finalResult = result
                    if let saveAs = step.saveAs {
                        stepOutputs[saveAs] = result
                    }

                case "image":
                    let result = try await aiService.generateImage(model: imageModelName,
                                                                   apiKey: apiKey,
                                                                   prompt: prompt)
                    ...
                    finalImages.append(imageRef)
                    if let saveAs = step.saveAs {
                        // We can't save an image as a string directly for the next step,
                        // but we can save its prompt or a description.
                        stepOutputs[saveAs] = "An image was generated for the prompt: \(prompt)"
                    }
                    
                case "web_search":
                    let items = try await webSearchService.search(query: prompt)
                    let searchSummary = formatSearchResults(items)
                    finalResult = searchSummary
                    if let saveAs = step.saveAs {
                        stepOutputs[saveAs] = searchSummary
                    }

                default:
                    throw AIService.AIServiceError.invalidResponse // Or a more specific error
                }
            } catch {
                if await handleChatCancellation(replyId: replyId, error: error) { return }
                await MainActor.run {
        ...
        // Finalize the chat reply
        await MainActor.run {
            if let finalResult {
                setChatReplyText(replyId: replyId, text: finalResult)
            }
            if !finalImages.isEmpty {
                setChatReplyImages(replyId: replyId, images: finalImages)
            }
            finishChatReply(replyId: replyId)
        }
    }

    private func getPlan(userText: String,
                         images: [ImageRef],
                         files: [FileRef],
                         apiKey: String,
                         history: [ChatMsg],
                         ...
                         selection: Set<UUID>,
                         personality: String,
                         userName: String) async throws -> Plan {
        ...
        guard let json = extractJSONObject(from: routerOutput),
        ...
        do {
            ...
            // If decoding fails, try to extract the json from a markdown block
            if let extractedJson = extractJsonFromMarkdown(from: routerOutput),
               let extractedData = extractedJson.data(using: .utf8),
               let plan = try? JSONDecoder().decode(Plan.self, from: extractedData) {
                return plan
            }
    ...
    private func extractJsonFromMarkdown(from text: String) -> String? {
        let pattern = "```json\\n([\\s\\S]*?)\\n```"
        do {
            ...
            print("Error extracting json from markdown: \(error)")
        }
        return nil
    }

    private func substituteVariables(in prompt: String, with outputs: [String: String]) -> String {
        var finalPrompt = prompt
        for (key, value) in outputs {
            finalPrompt = finalPrompt.replacingOccurrences(of: "{{\(key)}}", with: value)
        }
        return finalPrompt
    }
