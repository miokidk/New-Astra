...
final class BoardStore: ObservableObject {
    ...
    @Published var liveReasoningSummary: String = ""
    @Published var liveReasoningTokens: Int?

    private var reasoningByReply: [UUID: (summary: String, tokens: Int?)] = [:]
...
extension BoardStore {
    ...
    func addThought(_ summary: String, related: [UUID]? = nil, relatedChatId: UUID? = nil, reasoningTokens: Int? = nil) {
...
extension BoardStore {
    ...
    func sendChat(text: String, image: ImageRef? = nil) -> Bool {
        ...
        Task { [weak self] in
            guard let self else { return }
            do {
                reasoningByReply[replyId] = ("", nil)
                liveReasoningSummary = ""
                liveReasoningTokens = nil

                try await aiService.streamChat(
                    apiKey: apiKey,
                    model: model,
                    systemPrompt: systemPrompt,
                    messages: openAIMessages,
                    onTextDelta: { delta in
                        Task { @MainActor in self.appendChatDelta(delta, replyId: replyId) }
                    },
                    onReasoningSummaryDelta: { delta in
                        Task { @MainActor in
                            var cap = self.reasoningByReply[replyId] ?? ("", nil)
                            cap.summary += delta
                            self.reasoningByReply[replyId] = cap
                            self.liveReasoningSummary = cap.summary
                        }
                    },
                    onReasoningTokens: { tokens in
                        Task { @MainActor in
                            var cap = self.reasoningByReply[replyId] ?? ("", nil)
                            cap.tokens = tokens
                            self.reasoningByReply[replyId] = cap
                            self.liveReasoningTokens = tokens
                        }
                    }
                ) { [weak self] delta in
                    guard let self else { return }
    ...
    let cap = reasoningByReply.removeValue(forKey: replyId)
    liveReasoningSummary = ""
    liveReasoningTokens = nil

    if let cap, !cap.summary.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        addThought(
            cap.summary.trimmingCharacters(in: .whitespacesAndNewlines),
            related: selection.isEmpty ? nil : Array(selection),
            relatedChatId: replyId,
            reasoningTokens: cap.tokens
        )
    } else {
        addModelThought() // fallback if the model didn’t provide a summary
    }
