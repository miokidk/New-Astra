...
struct EntryContainerView: View {
    ...
    private func selectionOverlay(rect: CGRect) -> some View {
        ZStack(alignment: .topLeading) {
            if isSelected {
                if entry.type == .text {
                    selectionOutlineView(color: Color.gray.opacity(0.6),
            ...
            if isSelected && (entry.type == .shape || entry.type == .text) {
                Button(action: {
    ...
    var body: some View {
        ZStack {
            ...
            case .text(let text):
                ...
                let zoom = store.doc.viewport.zoom.cg
                AutoSizingTextView(text: Binding(get: {
                    activeTextEdit == entry.id ? text : text
                }, set: { new in
                    store.updateText(id: entry.id, text: new)
                }), style: textStyle, isEditable: activeTextEdit == entry.id, zoom: zoom)
        ...
        .background(Color.white.opacity(entry.type == .text ? 0.0 : 0.08))
    }
...
private struct AutoSizingTextView: NSViewRepresentable {
    ...
    private func applyStyle(_ textView: NSTextView) {
        ...
        let textColor = NSColor(calibratedRed: style.textColor.red.cg,
                                green: style.textColor.green.cg,
                                blue: style.textColor.blue.cg,
                                alpha: style.textOpacity.cg)
        let outlineColor = NSColor(calibratedRed: style.outlineColor.red.cg,
