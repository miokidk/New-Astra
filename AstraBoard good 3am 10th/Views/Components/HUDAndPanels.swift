...
struct MemoriesPanelView: View {
    ...
    private func rebuildFindMatches(_ memories: [String]) {
        let q = findQuery.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !q.isEmpty else {
            findMatches = []
            findIndex = 0
        ...
        let hits = memories.enumerated().compactMap { idx, mem in
            mem.localizedCaseInsensitiveContains(q) ? idx : nil
        }
    ...
    private func applyFindCommand(_ cmd: FindCommand, memories: [String], proxy: ScrollViewProxy) {
        switch cmd {
        case .open:
            isFindVisible = true
                        ...
                        if memories.isEmpty {
                            ...
                            ScrollView {
                                LazyVStack(spacing: 0) {
                                    ForEach(Array(memories.enumerated()), id: \.offset) { idx, mem in
                                        HStack(alignment: .top, spacing: 10) {
                                            Text(mem)
                                                .frame(maxWidth: .infinity, alignment: .leading)
...
struct LogPanelView: View {
    ...
    private var chatLogs: [LogItem] {
        store.doc.log.filter { $0.relatedChatId != nil }.sorted { $0.ts > $1.ts }
    }

    private func chatPreview(for chat: ChatThread?) -> String {
        if let chat, let chatTitle = chat.title, !chatTitle.isEmpty {
            return chatTitle
        }
            ...
            if chatLogs.isEmpty {
                ...
                ScrollView {
                    LazyVStack(spacing: 0, pinnedViews: []) {
                        ForEach(chatLogs) { item in
                            if let chatId = item.relatedChatId {
                                let chat = store.archivedChat(id: chatId)
                                VStack(alignment: .leading, spacing: 6) {
                                    let title = (chat != nil) ? chatPreview(for: chat) : (item.summary.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? "Chat" : item.summary)
                                    Text(title)
                                    ...
                                    HStack {
                                        Text(Date(timeIntervalSince1970: item.ts), style: .date)
                                            .font(.caption)
                                            .foregroundColor(.secondary)
